...
static void thread_init(void)
{
	...
	DMX_DEBUG(1, "Register thread \"main\"");
}
...
static void* thread_entry(void *arg)
{
	...
	DMX_DEBUG(1, "Register thread \"%s\" %p", th->name, (void*)th->thread);
		...
		const char *name)
{
	...
	if(ret)
	{
		DMX_DEBUG(1, "create thread failed");
		pthread_mutex_lock(&lock);
		thread_remove(th);
		pthread_mutex_unlock(&lock);
...
void AM_pthread_exit(void *r)
{
	...
	if(th)
		...
		DMX_DEBUG(1, "thread %p is not registered", (void*)pthread_self());
...
int AM_pthread_leave(const char *file, const char *func, int line)
{
	...
	{
		if(!th->frame_top)
			DMX_DEBUG(1, "AM_pthread_enter and AM_pthread_leave mismatch");
		else
			th->frame_top--;
	}
	else
