...
/**\brief 全局锁*/
static pthread_mutex_t am_gDmxLock = PTHREAD_MUTEX_INITIALIZER;
...
static AM_INLINE AM_ErrorCode_t dmx_get_dev(int dev_no, AM_DMX_Device_t **dev)
{
	...
	{
		DMX_DEBUG(1, "invalid demux device number %d, must in(%d~%d)", dev_no, 0, DMX_DEV_COUNT-1);
...
static AM_INLINE AM_ErrorCode_t dmx_get_openned_dev(int dev_no, AM_DMX_Device_t **dev)
{
	...
	if((*dev)->open_count <= 0)
	{
		DMX_DEBUG(1, "demux device %d has not been openned", dev_no);
		return AM_DMX_ERR_INVALID_DEV_NO;
	}
...
static AM_INLINE AM_ErrorCode_t dmx_get_used_filter(AM_DMX_Device_t *dev, int filter_id, AM_DMX_Filter_t **pf)
{
	...
	{
		DMX_DEBUG(1, "invalid filter id, must in %d~%d", 0, DMX_FILTER_COUNT-1);
	...
	if(!filter->used)
	{
		DMX_DEBUG(1, "filter %d has not been allocated", filter_id);
	...
	while(dev->enable_thread)
	{
		...
		if(ret==AM_SUCCESS)
		{
			...
			for(id=0; id<DMX_FILTER_COUNT; id++)
			{
				...
				{
					if(id && sec)
					DMX_DEBUG(5, "filter %d data callback len fd:%ld len:%d, %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
					...
					if(id && sec)
					DMX_DEBUG(5, "filter %d data callback ok", id);
				}
			}
#if defined(DMX_WAIT_CB) || defined(DMX_SYNC)
			pthread_mutex_lock(&dev->lock);
...
AM_ErrorCode_t AM_DMX_Open(int dev_no, const AM_DMX_OpenPara_t *para)
{
	...
	pthread_mutex_lock(&am_gDmxLock);
	
	if(dev->open_count > 0)
	{
		DMX_DEBUG(1, "demux device %d has already been openned", dev_no);
		dev->open_count++;
		ret = AM_SUCCESS;
		goto final;
	}
...
final:
	pthread_mutex_unlock(&am_gDmxLock);
...
AM_ErrorCode_t AM_DMX_AllocateFilter(int dev_no, int *fhandle)
{
	...
	{
		DMX_DEBUG(1, "no free section filter");
...
AM_ErrorCode_t AM_DMX_SetSecFilter(int dev_no, int fhandle, const struct dmx_sct_filter_params *params)
{
	...
	{
		DMX_DEBUG(1, "demux do not support set_sec_filter");
	...
	if(ret==AM_SUCCESS)
	{
		...
		DMX_DEBUG(5, "set sec filter %d PID: %d filter: %02x:%02x %02x:%02x %02x:%02x %02x:%02x %02x:%02x %02x:%02x %02x:%02x %02x:%02x",
				fhandle, params->pid,
...
AM_ErrorCode_t AM_DMX_SetPesFilter(int dev_no, int fhandle, const struct dmx_pes_filter_params *params)
{
	...
	{
		DMX_DEBUG(1, "demux do not support set_pes_filter");
		return AM_DMX_ERR_NOT_SUPPORTED;
	}
	...
	if(ret==AM_SUCCESS)
	{
		...
		DMX_DEBUG(2, "set pes filter %d PID %d", fhandle, params->pid);
	}
...
AM_ErrorCode_t AM_DMX_SetBufferSize(int dev_no, int fhandle, int size)
{
	...
	{
		DMX_DEBUG(1, "do not support set_buf_size");
...
AM_ErrorCode_t AM_DMX_SetSource(int dev_no, AM_DMX_Source_t src)
{
	...
	{
		DMX_DEBUG(1, "do not support set_source");
		ret = AM_DMX_ERR_NOT_SUPPORTED;
	}
...
#if 0
static AM_ErrorCode_t AM_FileRead(const char *name, char *buf, int len)
{
	FILE *fp;
	char *ret;
	...
	if(!fp)
	{
		DMX_DEBUG(1, "cannot open file \"%s\"", name);
		return AM_FAILURE;
	}
	...
	if(!ret)
	{
		DMX_DEBUG(1, "read the file:\"%s\" error:\"%s\" failed", name, strerror(errno));
	}
...
AM_ErrorCode_t AM_DMX_GetScrambleStatus(int dev_no, AM_Bool_t dev_status[2])
{
	...
	for (i=0; i<5; i++)
	{
		if(AM_FileRead(class_file, buf, sizeof(buf))==AM_SUCCESS)
		{
			...
			//DMX_DEBUG(1, "AM_DMX_GetScrambleStatus video scamble %d, audio scamble %d\n", vflag, aflag);
		...
		{
			DMX_DEBUG(1, "AM_DMX_GetScrambleStatus read scamble status failed\n");
			return AM_FAILURE;
		}
	}
