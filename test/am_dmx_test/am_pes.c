...
AM_ErrorCode_t AM_PES_Decode(AM_PES_Handle_t handle, uint8_t *buf, int size)
{
	...
	do {
		...
		if (plen == 0xffff || plen == 0x0) {
			invalid = 1;
			DMX_DEBUG(1, "pes packet plen error plen[%d]parser->len[%d]", plen, parser->len);
		}
			...
			if (stream_id != PROGRAM_STREAM_MAP && stream_id != PRIVATE_STREAM_2 &&
				...
				if (parser->para.afmt == AFORMAT_AC3 || parser->para.afmt == AFORMAT_EAC3) {
					...
					if (start_code != AC3_PAYLOAD_START_CODE_1 && start_code != AC3_PAYLOAD_START_CODE_2) {
						// skip this pes data
						DMX_DEBUG(1, "pes packet skip not ac3 data [0x%x]", start_code);
						plen = 0;
						goto skip;
					}
				}
				...
				if (parser->para.afmt == AFORMAT_AC3 || parser->para.afmt == AFORMAT_EAC3) {
					...
					if (start_code != AC3_PAYLOAD_START_CODE_1 && start_code != AC3_PAYLOAD_START_CODE_2) {
						// skip this pes data
						DMX_DEBUG(1, "pes packet skip not ac3 data payload [0x%x] ", start_code);
						plen = 0;
						goto skip;
					}
				}
		...
		if (parser->para.payload_only) {
			if (invalid == 1) {
				...
				if (calc_invalid_pes_offset(parser->buf +pos, parser->len - pos, &plen, &dpos, &dlen) != 0) {
					DMX_DEBUG(1, "pes packet calc error");
					goto end;
				} else {
					DMX_DEBUG(1, "pes packet calc plen=[%d]dpos[%d]dlen[%d]", plen, dpos, dlen);
				}
			} else {
				int stream_id = parser->buf[pos+3] | PESPACKET_STREAMID_MASK;
				if (stream_id == PADDING_STREAM) {
					/* padding_stream,need skip padding stream */
					DMX_DEBUG(1, "pes packet skip padding");
					goto skip;
				}
				/* program_stream_map, private_stream_2 */
				/* ECM, EMM */
				...
				if (stream_id != PROGRAM_STREAM_MAP && stream_id != PRIVATE_STREAM_2 &&
					...
					dlen = plen + 6 - hlen;
					/* skip stuf data*/
					DMX_DEBUG(2, "pes packet has head");
				} else {
					/*payload case*/
					hlen = 6;
					dpos = pos + hlen;
					dlen = plen + 6 - hlen;
					DMX_DEBUG(1, "pes packet payload");
				}
				if (0) {
					if ((parser->buf[pos+6] & 0xC0) == 0x80) {
						hlen = 6 + 3 + parser->buf[pos+8];
					} else {
						DMX_DEBUG(1, "pes packet assume MPEG-1 [%#x][%#x]\n", parser->buf[pos+3], parser->buf[pos+6]);
		...
		if (parser->para.packet) {
			if (parser->len < dpos + dlen) {
				DMX_DEBUG(1, "pes packet parse error plen=[%d]dpos[%d]dlen[%d]", parser->len, dpos, dlen);
			} else {
