...
#define DVR_EVT_BUCKET_COUNT    50
...
/**\brief 事件*/
typedef struct DVR_Event DVR_Event_t;
struct DVR_Event
{
    DVR_Event_t        *next;    /**< 指向链表中的下一个事件*/
    DVR_EVT_Callback_t  cb;      /**< 回调函数*/
    int                type;    /**< 事件类型*/
    long               dev_no;  /**< 设备号*/
    void              *data;    /**< 用户回调参数*/
};
...
static DVR_Event_t *events[DVR_EVT_BUCKET_COUNT];
 ...
 *   - 其他值 错误代码
 */
int DVR_EVT_Subscribe(long dev_no, int event_type, DVR_EVT_Callback_t cb, void *data)
{
    DVR_Event_t *evt;
    int pos;
    int i;
    ...
    for (i = 0; i < DVR_EVT_BUCKET_COUNT; i++)
    ...
    /*分配事件*/
    evt = malloc(sizeof(DVR_Event_t));
    if (!evt)
    {
        DVR_DEBUG(1, "not enough memory");
        return DVR_EVT_ERR_NO_MEM;
    }
    evt->dev_no = dev_no;
    evt->type   = event_type;
    evt->cb     = cb;
    ...
    pos = event_type%DVR_EVT_BUCKET_COUNT;
 ...
 *   - 其他值 错误代码
 */
int DVR_EVT_Unsubscribe(long dev_no, int event_type, DVR_EVT_Callback_t cb, void *data)
{
    DVR_Event_t *evt, *eprev;
    int pos;
 ...
 * \param[in] param 事件参数
 */
int DVR_EVT_Signal(long dev_no, int event_type, void *param)
{
    DVR_Event_t *evt;
    int pos = event_type%DVR_EVT_BUCKET_COUNT;
    ...
    //5.但上层要避免在cb事件响应函数里面去DVR_EVT_Subscribe/DVR_EVT_Unsubscribe事件链表,因为这样会造成死锁.
    //6.不应该也不需要在cb函数里面去DVR_EVT_Subscribe/DVR_EVT_Unsubscribe事件链表,切记!!!
    pthread_rwlock_rdlock(&rwlock);
...
int DVR_EVT_Init()
{
    ...
    for (i = 0; i < DVR_EVT_BUCKET_COUNT; i++) events[i] = NULL;
    return 0;
}
/*
*/
