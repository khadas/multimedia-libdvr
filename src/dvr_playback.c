...
static int _do_check_pid_info(DVR_PlaybackHandle_t handle, DVR_StreamInfo_t  now_pid, DVR_PlaybackPids_t pids, int type);
...
static int dvr_playback_calculate_last_valid_segment(
  DVR_PlaybackHandle_t handle, uint64_t *segmentid, int *pos);
...
//get sys time ms
static uint64_t _dvr_time_getClock(void)
{
  struct timespec ts;
  uint64_t ms;
...
static int _dvr_init_fffb_t(DVR_PlaybackHandle_t handle) {
  ...
  DVR_PB_DG(1, " player->fffb_start:%lld", player->fffb_start);
...
static int _dvr_get_next_segmentId(DVR_PlaybackHandle_t handle) {
  ...
  if (IS_FB(player->speed)
    ...
    DVR_PB_DG(1, "has segment to fb play [%lld][%d]", segmentid, pos);
  }
...
static int _dvr_replay_changed_pid(DVR_PlaybackHandle_t handle) {
  ...
  //if (player->cmd.state == DVR_PLAYBACK_STATE_START)
  {
    ...
    _do_check_pid_info(handle, player->last_segment.pids.video, player->cur_segment.pids, 0);
    //check sub audio pids stop or restart
    _do_check_pid_info(handle, player->last_segment.pids.ad, player->cur_segment.pids, 2);
    //check audio pids stop or restart
    _do_check_pid_info(handle, player->last_segment.pids.audio, player->cur_segment.pids, 1);
...
static int _dvr_check_speed_con(DVR_PlaybackHandle_t handle)
{
  ...
  DVR_PB_DG(1, ":play speed: %f  ply dur: %lld sys_dur: %lld",
                player->speed,
      ...
      (player->play_flag&DVR_PLAYBACK_STARTED_PAUSEDLIVE) == DVR_PLAYBACK_STARTED_PAUSEDLIVE)
    {
      ...
      if (trick_stat > 0) {
        ...
        if (player->cmd.cur_cmd == DVR_PLAYBACK_CMD_SEEK || (player->play_flag&DVR_PLAYBACK_STARTED_PAUSEDLIVE) == DVR_PLAYBACK_STARTED_PAUSEDLIVE) {
          ...
          if (player->cmd.last_cmd == DVR_PLAYBACK_CMD_PAUSE
            ...
            if (player->state == DVR_PLAYBACK_STATE_PAUSE) {
              DVR_PB_DG(1, "fffb pause state----speed[%f] fffb cur[%lld] cur sys[%lld] [%s] [%lld]",
                            player->speed,
                            player->fffb_current,
                            _dvr_time_getClock(),
              ...
              DVR_PB_DG(1, "fffb timeout-to pause video---speed[%f] fffb cur[%lld] cur sys[%lld] [%s] [%lld]",
                            player->speed,
                            player->fffb_current,
                            _dvr_time_getClock(),
                            _dvr_playback_state_toString(player->state),
        ...
        if (player->state == DVR_PLAYBACK_STATE_PAUSE ||
          ...
          DVR_PB_DG(1, "fffb timeout-fffb play---speed[%f] fffb cur[%lld] cur sys[%lld] [%s] [%lld]",
                        player->speed,
                        player->fffb_current,
                        _dvr_time_getClock(),
                        _dvr_playback_state_toString(player->state),
...
check0:
      ...
      DVR_PB_DG(1, "write time out write_success:%d wbufs.buf_size:%d systime:%lld",
                    write_success,
                    wbufs.buf_size,
                    _dvr_time_getClock());
...
int dvr_playback_open(DVR_PlaybackHandle_t *p_handle, DVR_PlaybackOpenParams_t *params) {
  ...
  //init ff fb time
  player->fffb_current = UINT64_MAX;
  player->fffb_start = UINT64_MAX;
...
static int _do_check_pid_info(DVR_PlaybackHandle_t handle, DVR_StreamInfo_t  now_pid, DVR_PlaybackPids_t set_pids, int type) {
  DVR_Playback_t *player = (DVR_Playback_t *) handle;
    ...
    if (segment->segment_id == segment_id) {
      ...
      if (player->cur_segment_id == segment_id) {
        ...
        if (player->cmd.state == DVR_PLAYBACK_STATE_START) {
          pthread_mutex_unlock(&player->lock);
          //check video pids, stop or restart
          _do_check_pid_info((DVR_PlaybackHandle_t)player, segment->pids.video, *p_pids, 0);
          //check sub audio pids stop or restart
          _do_check_pid_info((DVR_PlaybackHandle_t)player, segment->pids.ad, *p_pids, 2);
          //check audio pids stop or restart
          _do_check_pid_info((DVR_PlaybackHandle_t)player, segment->pids.audio, *p_pids, 1);
          //check pcr pids stop or restart
          _do_check_pid_info((DVR_PlaybackHandle_t)player, segment->pids.pcr, *p_pids, 3);
...
{  DVR_Playback_t *player = (DVR_Playback_t *) handle;
  ...
  DVR_PB_DG(1, "time %lld limit: %d player->state:%d", time, limit, player->state);
  pthread_mutex_lock(&player->lock);
...
int dvr_playback_calculate_expiredlen(DVR_PlaybackHandle_t handle)
{
  ...
  //get system time
  cur_time = _dvr_time_getClock();
  if ((cur_time - player->rec_start) > player->limit)
    expired = cur_time - player->rec_start - player->limit;
  DVR_PB_DG(1, "cur_time:%lld, rec start:%lld limit:%d c_r_diff:%lld",
                cur_time,
                player->rec_start,
                player->limit,
                cur_time - player->rec_start);
  return expired;
}
...
//
  ...
  if (CONTROL_SPEED_ENABLE == 1) {
    if (player->con_spe.ply_sta == -1) {
        ...
        DVR_PB_DG(1, "player dur[%lld] sta[%lld] cur[%d]",
                      player->con_spe.ply_dur,
                      player->con_spe.ply_sta,
                      p_status->time_cur);
        player->con_spe.ply_sta = p_status->time_cur;
