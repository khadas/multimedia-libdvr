...
static PlayBack_Device playback_devices[PLAYBACK_DEV_COUNT] =
{
    {
        .isopen= 0
    }
 ...
 *   - DVR_SUCCESS
 */
int DVR_FileRead(const char *name, char *buf, int len)
{
	FILE *fp;
	char *ret;
...
static DVR_Bool_t check_vfmt_support_sched(AM_Playback_VFormat_t vfmt)
{
...
int playback_device_open(Playback_DeviceHandle *p_handle, Playback_DeviceOpenParams *params) {
    ...
    //dev->params
    memcpy(&dev->params, params, sizeof(Playback_DeviceOpenParams));
    //change and store dmx source
    DVR_FileRead(DVB_STB_SOURCE_FILE, dev->last_stb_src, 16);
    snprintf(buf, sizeof(buf), "dmx%d", dev->params.dmx);
    DVR_FileEcho(DVB_STB_SOURCE_FILE, buf);
    //change and store stb source
    snprintf(buf, sizeof(buf), DVB_STB_DEMUXSOURCE_FILE, dev->params.dmx);
    DVR_FileRead(buf, dev->last_dmx_src, 16);
    DVR_FileEcho(buf, "hiu");
...
int playback_device_close(Playback_DeviceHandle handle) {
    PlayBack_Device *dev = (PlayBack_Device *) handle;
    char buf[128];
    dev->isopen = 0;
    if (dev->fd != -1)
    {
    ...
    DVR_FileEcho(DVB_STB_SOURCE_FILE, dev->last_stb_src);
    snprintf(buf, sizeof(buf), DVB_STB_DEMUXSOURCE_FILE, dev->params.dmx);
    DVR_FileEcho(buf, dev->last_dmx_src);
